...
%  SS1m proof-battery receipt v2 — self-documenting integrity HUD
% ============================================================
...
% ════════════════════════════════════════════════════════════
%  SS1m PROOF-BATTERY RECEIPT — v2 (self-documenting integrity)
% ════════════════════════════════════════════════════════════
%
%  Every page footer carries a machine-readable proof-battery receipt.
%  NOTE: "proof battery closed" = all sub-tests in the theorem's
%  test suite pass.  This is NOT a PRE→POST canon weld in the
%  UMCP seam-closure sense (which requires two declared editions
%  under a frozen contract).  The seam objects here are
...
\texorpdfstring{\\}{: }A Measurable Structural Signature of Quark Binding
in the Generative-Collapse Kernel}
...
\date{\RevisionDate\enspace\textperiodcentered\enspace\WeldID}
...
The integrity bound~\eqref{eq:bound} is a Tier-44 structural identities derived independently from Axiom-0.  The classical
...
% ============================================================
\section{Frozen Contract Snapshot}\label{sec:contract}
% ============================================================

All results in this paper are computed under a frozen contract.
For canon continuity and one-command reproducibility, we state
the full contract header and normalization constants verbatim.

\subsection{Contract Identity}

\begin{table}[H]
\caption{Frozen contract header fields (verbatim from
\texttt{SM.INTSTACK.v1.yaml}, inheriting
\texttt{UMA.INTSTACK.v1}).}
\label{tab:contract}
\centering\small
\begin{tabular}{@{}ll@{}}
\toprule
...
\midrule
\texttt{contract.id}        & \texttt{SM.INTSTACK.v1} \\
\texttt{parent\_contract}    & \texttt{GCD.INTSTACK.v1} \\
\texttt{tier\_level}         & 1 \\
\texttt{version}             & \texttt{1.0.0} \\
\texttt{embedding.interval}  & $[0, 1]$ \\
\texttt{embedding.face}      & \texttt{pre\_clip} \\
\texttt{embedding.oor\_policy} & \texttt{clip\_and\_flag} \\
\texttt{embedding.epsilon}   & $10^{-8}$ (canon; T3 uses $\eps = 10^{-6}$) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\caption{Frozen kernel parameters (seam-derived, not chosen).
These are consistent across the seam; see \texttt{TIER\_SYSTEM.md}.}
\label{tab:frozen}
\centering\small
\begin{tabular}{@{}lll@{}}
\toprule
...
\midrule
$p$                & 3      & Minkowski exponent \\
$\alpha$           & 1.0    & Energy coupling \\
$\lambda$          & 0.2    & Decay rate \\
$\eta$             & 0.001  & Return-domain window \\
$\mathrm{tol}_{\rm seam}$ & 0.005  & Seam closure tolerance \\
$\mathrm{tol}_{\rm id}$   & $10^{-9}$ & Identity check tolerance \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Channel Normalization Constants}

The 8-channel trace vector uses the following normalization
constants, all frozen per run:

\begin{table}[H]
\caption{Channel normalization constants (from
\texttt{subatomic\_kernel.py}).  These define the exact mapping
from physical observables to $c_i \in [\eps, 1{-}\eps]$.}
...
\midrule
$m_{\rm floor}$   & $10^{-11}$\,GeV  & mass\_log \\
$m_{\rm ceil}$    & $200.0$\,GeV     & mass\_log \\
$\tau_{\rm Pl}$   & $5.39{\times}10^{-44}$\,s & stability \\
$\tau_{\rm univ}$ & $4.35{\times}10^{17}$\,s & stability \\
$Y_{\rm max}$     & $\max|Y_i/2|$   & hypercharge \\
$\log_2(9)$       & ${\approx}\,3.17$& color\_dof \\
\midrule
$\eps$ (paper)    & $10^{-6}$        & guard band \\
\bottomrule
\end{tabular}
\end{table}
...
Equal weights: $w_i = 1/n = 1/8$ for all 8 channels.
This is the unique uninformative prior; any deviation would
require a physics-motivated argument that has not been made.

\begin{remark}[Contract vs.\ Paper Guard Band]\label{rem:epsilon}
The parent contract \texttt{UMA.INTSTACK.v1} freezes
$\eps = 10^{-8}$.  This paper uses $\eps = 10^{-6}$ as the
default, demonstrated robust across $[10^{-12}, 10^{-2}]$
(Table~\ref{tab:epsilon}).  The 14/14 bound holds at
\emph{every} value, including the canon $10^{-8}$.
\end{remark}

\subsection{Constituent Quark Masses}

The binding fraction channel uses constituent (not current)
quark masses~\cite{pdg2024}:
\begin{center}\small
\begin{tabular}{cccccc}
$u$ & $d$ & $s$ & $c$ & $b$ & $t$ \\
\midrule
0.336 & 0.340 & 0.486 & 1.55 & 4.73 & 172.69 \\[2pt]
\multicolumn{6}{c}{(GeV, constituent masses)}
\end{tabular}
\end{center}
...
% ============================================================
\section{Reproducibility: CasePack Bundle}\label{sec:casepack}
% ============================================================

All results in this paper are reproducible via a single command.
The reference implementation ships a CasePack-style bundle
(\texttt{casepacks/confinement\_T3/}) containing:

\begin{enumerate}
\item \textbf{PDG raw table}: the 31 particles (17~fundamental +
  14~composite) with all quantum numbers and mass/lifetime data
  transcribed from PDG~2024~\cite{pdg2024}
  (\texttt{raw\_particles.csv}).
\item \textbf{Constructed trace vectors}: the 8-channel normalized
  vectors for each particle, computed under the frozen contract
  (\texttt{expected/trace\_vectors.json}).
\item \textbf{Run manifest}: contract reference, closure paths,
  frozen parameters, and expected outputs
  (\texttt{manifest.json}).
\item \textbf{Script entrypoint}: a single Python script that
  reads the raw table, constructs trace vectors, computes kernel
  invariants, verifies Theorem~T3 (19/19 sub-tests), and
  produces a conformance receipt
  (\texttt{run\_confinement\_T3.py}).
\end{enumerate}

\begin{tcolorbox}[
  colback=PreambleBox, colframe=EpsBlue,
  title=One-Command Rerun, fonttitle=\bfseries\small,
  boxrule=0.8pt, arc=2pt, left=8pt, right=8pt
]
\small\ttfamily
\$ cd casepacks/confinement\_T3 \\
\$ python run\_confinement\_T3.py \\[2pt]
\rmfamily\small
Produces: kernel invariants for all 31 particles,
Theorem~T3 verdict (19/19 sub-tests), guard band
robustness table, and exotic hadron verification.
All frozen parameters are sourced from the contract,
not hardcoded in the script.
\end{tcolorbox}

The CasePack references \texttt{SM.INTSTACK.v1.yaml}
(Sec.~\ref{sec:contract}) and the closure
\texttt{closures/standard\_model/subatomic\_kernel.py}.
...
% ── proof-battery receipt summary ─────────────────────────────
\begin{tcolorbox}[
  colback=white, colframe=WeldGreen,
  title={\small SS1m Proof-Battery Receipt — \WeldID},
  fonttitle=\bfseries\small,
  boxrule=1pt, arc=2pt, left=4pt, right=4pt
]
\scriptsize
\begin{tabular}{@{}ll@{\,}ll@{}}
...
Proof battery closed.
\\[4pt]
{\scriptsize\textit{Note: ``proof battery closed'' = all theorem sub-tests pass.
This is a single-edition verification, not a PRE$\to$POST canon
weld (which requires two declared editions under a frozen contract).}}
\end{tcolorbox}
