...
operational state of RCFT as a \emph{Tier-23 domains expansion}
...
The system is validated across 246 closure modules domains,
...
\begin{enumerate}[nosep,leftmargin=*]
  ...
  \item A \textbf{self-contained presentation} of RCFT as
    a Tier-23 domains expansion on GCD, with all necessary
...
$\eta$ & $0.001$ & Return proximity &
  Distance threshold in $\|\Psifield(t) - \Psifield(u)\| \leq \eta$ for return detection (Def.~\ref{def:tR}). A state ``returns'' only if it re-enters within $\eta$ of a prior state. Too large $\to$ false returns; too small $\to$ no returns detected. Discovered as the value giving consistent $\tR$ across 23 domains. \\
\midrule
\multicolumn{4}{@{}l}{\textit{Seam Tolerance}} \\[2pt]
$\tolseam$ & $0.005$ & Seam tolerance &
  Maximum residual $|s| \leq \tolseam$ for a seam to close. The integrity bound $\IC \leq \Fid$ holds at 100\% at this tolerance across 23 domains and 146 experiments. Tighter values cause false nonconformance from floating-point noise; looser values admit genuinely broken seams. \\
\midrule
...
across 23 domains and 146 experiments.
...
RCFT is not the whole theory. It is a Tier-23 domains
expansion on GCD.
...
belongs to a well-defined universality class.
All 246 closure modules domains share the same exponents.
\end{remark}
...
gates, zero tests became 3{,}27 tests, zero domains
became 23 domains.
...
trace vectors.
Seven theorems (T-EC-1 to T-EC-7, 95/27 tests) prove that
...
(L28--L34), and extensions (L35--L47).
...
\subsection{Extensions (Lemmas 35--47)}
...
\begin{description}[nosep,style=unboxed,leftmargin=0pt]
  ...
  \item[L47 (Geometric Slaughter, Cross-Domain):]
    One dead channel $c_k \to \eps$ kills $\IC$ while
    $\Fid$ stays healthy: $\IC/\Fid \to \eps^{w_k}
    \cdot (\IC_{\mathrm{rest}}/\Fid)$.
    Observed in 8 of 23~domains at phase boundaries
    (confinement, electroweak symmetry breaking,
    cortical lesion); empirically:
    neutron $\IC/\Fid = 0.0089$ and proton
    $\IC/\Fid = 0.0371$ (dead colour channel).
\end{description}

\begin{remark}
Together, Lemmas 1--47 form the \emph{proof substrate}
...
23 domains, 246 closure modules, all \conform{} &
...
\begin{enumerate}[nosep,leftmargin=*]
  ...
  \item \textbf{Black hole entropy via surface integration.}
    Remains a future Tier-246 closure modules candidate
    ($\tR = \infty_{\mathrm{rec}}$).
\end{enumerate}
...
\begin{enumerate}[nosep,leftmargin=*]
  \item \textbf{Black hole entropy via collapse memory}:
    ...
    requires a gravitational Tier-246 closure modules with observational data.
...
cross-domain validation across 23 domains (Sec.~\ref{sec:universality}),
...
validated across 23 domains, and welded onto the frozen
anchor of the original publication.

Key results:
\begin{enumerate}[nosep,leftmargin=*]
  \item RCFT is properly situated as a \textbf{Tier-23 domains
...
Tier-246 closure modules. The system's architecture —
immutable Tier-1 identities, frozen Tier-0 protocol,
extensible Tier-246 closure modules — is designed precisely
to absorb these additions without structural regression.

The reference implementation, all 24+ theorems,
3{,}27 tests, and the full closure architecture are
available at
