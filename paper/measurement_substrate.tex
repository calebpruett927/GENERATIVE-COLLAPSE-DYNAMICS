...
$\IC/F$ and noise budget $\langle\Delta\rangle$ by scale, with
the Bernoulli regime and dominant physical noise type at each rung.
...
The expanded Table~\ref{tab:coherence-ratio} reveals a further
structural fact: reality distinguishes itself from noise because the
Fisher metric $\gF(c) = 1/[c(1{-}c)]$ assigns infinite precision to
the boundaries ($c \to 0$, $c \to 1$) where collapse is certain, and
finite precision --- minimum $\gF = 4$ at $c = 1/2$ --- to the
interior where uncertainty is maximal.

The Bernoulli variance $c(1{-}c)$ is the per-channel noise.  It is
not a nuisance to be subtracted but a \emph{structural component} of
measurement: the noise IS the interior of the Bernoulli manifold.
The heterogeneity gap $\Delta = F - \IC$ is then the
\emph{noise budget} at each scale --- it measures how much
per-channel variance destroys multiplicative coherence.

Two regimes emerge from the data:
\begin{itemize}
\item \textbf{Poisson-like} ($\langle c \rangle < 0.25$):  Only the
  Planck rung occupies this regime.  The Fisher metric is elevated
  ($\gF = 36.1$), reflecting that near-boundary channels carry high
  precision but are fragile --- one dead channel is instantly
  detectable.  This is the regime of shot noise and quantum
  vacuum fluctuations.

\item \textbf{Symmetric Bernoulli} ($0.25 \leq \langle c \rangle
  \leq 0.75$):  All other rungs.  The Fisher metric clusters near
  its minimum ($\gF \approx 4$--$5$), reflecting maximum per-channel
  variance.  This is the regime of thermal (Johnson--Nyquist) noise,
  $1/f$ noise, and partition fluctuations.  The physical mechanism
  changes with scale --- Johnson noise at the everyday scale,
  thermonuclear shot noise in stellar interiors, cosmic variance at
  the horizon --- but the Bernoulli geometry is identical.
\end{itemize}

The measurement substrate is therefore simultaneously a measurement
geometry \emph{and} a noise geometry.  The Fano-Fisher duality
$h''(c) = -\gF(c)$ (Eq.~\ref{eq:fano-fisher}) ties the curvature of
entropy to the curvature of distinguishability, and the noise budget
$\Delta$ measures how the interplay between these two curvatures
determines coherence at each scale.
...
\item The noise budget $\langle\Delta\rangle$ at each scale
  is governed by the Bernoulli variance $c(1{-}c)$ and
  the Fisher metric $\gF = 1/[c(1{-}c)]$, which assign
  infinite precision to the boundaries and minimum precision
  to the equator --- reality distinguishes itself from noise
  because the geometry makes boundaries infinitely
