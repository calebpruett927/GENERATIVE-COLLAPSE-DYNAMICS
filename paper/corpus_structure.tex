...
44~structural identities, 47~proven lemmas, 5~frozen parameters,
5~structural constants, 4~regime gates, and 3~algebraic
identities that constitute the Tier-1 kernel and its Tier-0
protocol (reference implementation version~2.3.3).  The corpus
comprises 20{,}540~automated tests across 23~domain closures and
246~closure modules, $\sim$620~formally tagged objects catalogued
by tier and lineage, and 406~scale-ladder entities spanning
61~orders of magnitude.  The Tier-0 protocol is dually realised in
Python and in a portable C99 orchestration core ($\sim$1{,}900
lines, 326~test assertions) with an optional C++17 accelerator
(434~Catch2 assertions) bound to Python via pybind11, producing a
stable \texttt{extern "C"} ABI for the full validation spine.
\end{abstract}

\maketitle

% ============================================================
% Reader's Route — fast-access map to the canonical reference
% ============================================================
\begin{figure*}[!t]
\centering
\begin{tikzpicture}
\node[draw=blue!55!black, line width=0.7pt,
      fill=blue!4, rounded corners=4pt,
      inner sep=11pt, text width=0.96\textwidth] (route) {%
\renewcommand{\arraystretch}{1.22}%
\setlength{\tabcolsep}{8pt}%
\begin{minipage}{\linewidth}
\centering
{\bfseries\large\color{blue!55!black}%
  Reader's Route\,---\,Where to start}\\[1pt]
{\footnotesize\itshape\color{blue!55!black}%
  This is a canonical reference. New readers may prefer the
  \href{https://github.com/calebpruett927/GENERATIVE-COLLAPSE-DYNAMICS/blob/main/scripts/orientation.py}%
  {Orientation Series}
  or
  \href{https://github.com/calebpruett927/GENERATIVE-COLLAPSE-DYNAMICS/blob/main/THE_FIRST_PASS.md}%
  {First Pass} \\
  for an entry path; this paper is the spine they return to.}\\[6pt]
{\small
\begin{tabular}{@{}>{\bfseries\color{blue!55!black}}l@{\hspace{0.6em}}l@{\hspace{1.4em}}>{\bfseries\color{blue!55!black}}l@{\hspace{0.6em}}l@{}}
If you need\,\dots & go to & If you need\,\dots & go to \\
\midrule
the axiom               & \S\ref{sec:axiom}\,(II)
  & return / welds        & \S\ref{sec:seam}\,(VII), \S\ref{sec:governance}\,(XV) \\
the kernel              & \S\ref{sec:kernel}\,(IV)
  & implementation scope  & \S\ref{sec:scope}\,(XVI) \\
constants \& gates      & \S\hyperref[sec:regime]{V}\,--\,\S\ref{sec:regime}\,(VI)
  & the spine end-to-end  & \S\ref{sec:canon}\,(XII) \\
\end{tabular}}
\end{minipage}
};
\end{tikzpicture}
...
methods.
Section~\ref{sec:lemmas} inventories the 47~proven lemmas.
...
The corpus comprises exactly three tiers.
Every symbol, function, artifact, claim, and definition belongs
to exactly one tier.  This structure is the current validated
...
$\IC = e^{-0.3648} = 0.6942$,
$\Delta = 0.7 - 0.6942 = 0.0058$---within 22\% of the
perturbative estimate, as expected for $C = 0.20$ (the next
correction enters at $O(C^4)$).
\end{proof}
...
$\tolseam$ & $0.005$ & Seam tolerance & $\IC \leq F$ holds at 100\% across all 23 domains \\
\bottomrule
\end{tabular}}
\end{center}
...
\item $\tolseam = 0.005$ is the width where $\IC \leq F$ holds at
  $100\%$ across all 23~domain closures and 406~scale-ladder
...
kernel's structural behavior across all 23~tested domains:
\begin{itemize}[leftmargin=*]
...
tightest thresholds at which all 23~domain closures and
...
\subsection{Extended lemmas (L35--L47)}
...
L47 & Geometric slaughter (cross-domain) & One dead channel $c_k \to \eps$ kills $\IC$ while $F$ stays healthy: $\IC/F \to \eps^{w_k} \cdot (\IC_\mathrm{rest}/F)$.  Observed in 8/23 domains at phase boundaries (confinement, EWSB, cortical lesion). \\
\bottomrule
\end{tabular}}
\end{center}

\paragraph{Verification.}
The 47~proven lemmas are not conjectures; each is verified
\emph{computationally} across the full test suite
(20{,}540~tests across 233~test files).  Range lemmas (L1--L4)
...
suite.  The cross-domain geometric-slaughter lemma (L47)
is verified by the orientation script (section~3) and the
confinement-cliff observation in the standard-model closure
(neutron $\IC/F = 0.0089$, proton $\IC/F = 0.0371$; all
hadrons below $0.04$ owing to the dead colour channel).
The full lemma verification is reproducible:
\texttt{pytest tests/} exercises every lemma through
parametrized tests spanning all 23~domains.
...
mechanism by which 23~domains speak to each other through a
shared grammar.
...
The architecture has been validated across 23~domain closures,
20{,}540~automated tests across 233~test files, and 406~scale-ladder
objects spanning 61~orders of magnitude.

\subsection{The 23 domains}
...
19 & Clinical neuroscience & 10-channel cortical kernel \\
20 & Spacetime memory & 40 entities, budget-surface kernel \\
21 & Ecology & Population/community closures \\
22 & Immunology & Adaptive-immunity closures \\
23 & Information theory & Channel/coding closures \\
\bottomrule
\end{tabular}
\end{center}
...
not interpretive.  The 23~domains draw on established
...
\midrule
Reference implementation & v2.3.3 \\
Automated tests & 20{,}540 \\
Test files & 233 \\
Structural identities & 44 \\
Lemmas & 47 \\
Formally tagged objects ($\sim$) & 620 \\
Domain theorems (cumulative) & 746 \\
Domain closures & 23 \\
Closure modules & 246 \\
Casepacks & 26 \\
...
Papers (LaTeX) & 24 \\
Bibliography entries & 200 \\
C99 orchestration assertions & 326 \\
C++ accelerator assertions & 434 \\
\bottomrule
\end{tabular}}
\end{center}
...
identity tests spanning all 23~domains (reproduced by the
...
residual closure ($|s| > \tolseam$).
Each failure mode is specific and reproducible~\cite{umcpmetadatarepo}.
...
The grammar has been validated across 23~domains, 406~objects,
61~orders of magnitude, and 20{,}540~automated tests.  It produces
...
maintained in the UMCP reference implementation~\cite{umcpmetadatarepo}
and described in~\cite{paulus2025umcp,paulus2025episteme}.
\end{acknowledgments}
...
\begin{tikzpicture}[
  tier/.style={
    ...
    minimum width=13cm, minimum height=3.0cm,
    text width=12.3cm, align=center,
    line width=1.0pt
  },
  fwd/.style={-{Stealth[length=9pt,width=6pt]}, line width=1.5pt},
  back/.style={-{Stealth[length=7pt,width=5pt]}, line width=1.0pt,
    red!60!black, densely dashed}]

%% --- Tier-1 ---
\node[tier, fill=blue!13, draw=blue!55!black] (t1) at (0,8.2) {
  \textbf{\large Tier-1\;|\;The Kernel}\\[5pt]
  {\normalsize $K:\,[0,1]^n \!\times\! \Delta^n
    \;\longrightarrow\;
    \bigl(F,\;\omega,\;S,\;C,\;\kappa,\;\IC\bigr)$}\\[3pt]
  {\small 47~proven lemmas\enspace$\cdot$\enspace
    44~structural identities\enspace$\cdot$\enspace
    3~effective DOF ($F$,\,$\kappa$,\,$C$)}\\[1pt]
  {\small 5~frozen constants
    ($\varepsilon$,\,$p$,\,$\alpha$,\,$\lambda$,\,$\mathrm{tol}$)\enspace$\cdot$\enspace
    \textbf{domain-independent, immutable}}
};

%% --- Tier-0 ---
\node[tier, fill=green!13, draw=green!55!black] (t0) at (0,4.1) {
  \textbf{\large Tier-0\;|\;Protocol}\\[5pt]
  {\normalsize embedding\enspace$\cdot$\enspace
    regime gates\enspace$\cdot$\enspace
    seam calculus\enspace$\cdot$\enspace
    contracts\enspace$\cdot$\enspace
    SHA-256}\\[4pt]
  {\small \textsc{conformant}\,/\,\textsc{nonconformant}\,/\,\textsc{non\_evaluable}%
    \enspace$\cdot$\enspace
    Python\,+\,C99 orchestration\,+\,C++ accelerator\enspace$\cdot$\enspace
    \textbf{frozen per run}}
};

%% --- Tier-2 ---
\node[tier, fill=orange!13, draw=orange!55!black] (t2) at (0,0) {
  \textbf{\large Tier-2\;|\;Expansion Space}\\[5pt]
  {\normalsize 23~domain closures\enspace$\cdot$\enspace
    channel selection\enspace$\cdot$\enspace
    entity catalogs\enspace$\cdot$\enspace
    normalization}\\[4pt]
  {\small 746~domain theorems\enspace$\cdot$\enspace
    20{,}540~automated tests\enspace$\cdot$\enspace
    61~orders of magnitude\enspace$\cdot$\enspace
    \textbf{freely extensible}}
};

%% --- Forward arrows (right of center) ---
\draw[fwd, blue!60!black]
  ([xshift=50pt]t1.south) -- ([xshift=50pt]t0.north)
  node[midway, right=7pt, font=\small, text=blue!70!black]
    {\emph{computes}};
\draw[fwd, green!60!black]
  ([xshift=50pt]t0.south) -- ([xshift=50pt]t2.north)
  node[midway, right=7pt, font=\small, text=green!70!black]
    {\emph{validates}};

%% --- Forbidden back-edges (left of center) ---
\draw[back]
  ([xshift=-50pt]t2.north) -- ([xshift=-50pt]t0.south)
  node[midway, font=\bfseries\normalsize,
    text=red!60!black, left=6pt] {\texttimes};
\draw[back]
  ([xshift=-50pt]t0.north) -- ([xshift=-50pt]t1.south)
  node[midway, font=\bfseries\normalsize,
    text=red!60!black, left=6pt] {\texttimes};

%% --- Footer ---
\node[font=\small, text=black!60, text width=13cm, align=center]
  at (0,-2.4)
  {One-way dependency within a frozen run:\;
   Tier-1\,$\to$\,Tier-0\,$\to$\,Tier-2.\;
   Back-edges (\texttimes) are structurally forbidden.\;
   Promotion Tier-2\,$\to$\,Tier-1 requires a formal seam weld across runs.};
...
\begin{tikzpicture}
  \def\totalw{13.0}
  \def\barh{1.8}
  \def\xs{1.625}    % Stable right edge  (12.5% of 13 cm)
  \def\xw{4.797}    % Watch right edge  (12.5+24.4 = 36.9%)

  %% --- Filled segments ---
  \fill[green!35, draw=green!65!black, line width=0.7pt]
    (0,0) rectangle (\xs,\barh);
  \fill[yellow!55!orange!20, draw=yellow!70!orange!40, line width=0.7pt]
    (\xs,0) rectangle (\xw,\barh);
  \fill[red!25, draw=red!55!black, line width=0.7pt]
    (\xw,0) rectangle (\totalw,\barh);

  %% --- Outer frame ---
  \draw[line width=0.9pt, black!75] (0,0) rectangle (\totalw,\barh);

  %% --- Threshold dividers (dashed, full bar height) ---
  \draw[line width=0.9pt, black!50, dashed]
    (\xs, 0) -- (\xs, \barh);
  \draw[line width=0.9pt, black!50, dashed]
    (\xw, 0) -- (\xw, \barh);

  %% --- Inside labels ---
  \node[font=\small, align=center] at (0.8125, 0.9) {
    \textbf{Stable}\\[2pt]
    {\normalsize 12.5\%}\\[1pt]
    {\scriptsize all 4 gates}};
  \node[font=\small, align=center] at (3.211, 0.9) {
    \textbf{Watch}\\[2pt]
    {\normalsize 24.4\%}\\[1pt]
    {\scriptsize partial}};
  \node[font=\small, align=center] at (8.899, 0.9) {
    \textbf{Collapse}\\[2pt]
    {\normalsize 63.1\%}\\[1pt]
    {\scriptsize $\omega \geq 0.30$}};

  %% --- Threshold ticks and labels below ---
  \draw[line width=0.7pt, black!55]
    (\xs, -0.1) -- (\xs, -0.45);
  \draw[line width=0.7pt, black!55]
    (\xw, -0.1) -- (\xw, -0.45);

  \node[font=\small, text=black!70, below]
    at (\xs, -0.45) {$\omega = 0.038$};
  \node[font=\small, text=black!70, below]
    at (\xw, -0.45) {$\omega = 0.30$};

  %% --- Gate condition annotation ---
  \node[font=\footnotesize, text=black!60, text width=13cm, align=center]
    at (6.5, -1.2)
    {Stable is conjunctive:
     $\omega < 0.038\;\wedge\;F > 0.90\;\wedge\;S < 0.15\;\wedge\;C < 0.14$
     \;(87.5\% of the manifold lies outside Stable).};
...
\begin{tikzpicture}[
  box/.style={
    ...
    minimum width=2.8cm, minimum height=2.0cm,
    text width=2.6cm, align=center,
    font=\small, line width=0.7pt, fill=#1
  },
  arr/.style={-{Stealth[length=7pt,width=5pt]}, line width=1.2pt, black!45},
  ann/.style={font=\scriptsize, text=black!60,
    text width=2.6cm, align=center}]

%% --- Five stops ---
\node[box=blue!14]    (c) at (0,0) {
  \textbf{Contract}\\[2pt]
  {\footnotesize freeze}\\[1pt]
  {\scriptsize sources, norms,\\[-1pt] thresholds}};
\node[box=green!14]   (n) at (3.6,0) {
  \textbf{Canon}\\[2pt]
  {\footnotesize tell}\\[1pt]
  {\scriptsize five words}};
\node[box=yellow!22]  (l) at (7.2,0) {
  \textbf{Closures}\\[2pt]
  {\footnotesize publish}\\[1pt]
  {\scriptsize threshold gates}};
\node[box=orange!16]  (g) at (10.8,0) {
  \textbf{Ledger}\\[2pt]
  {\footnotesize reconcile}\\[1pt]
  {\scriptsize debit/credit}};
\node[box=red!10]     (s) at (14.4,0) {
  \textbf{Stance}\\[2pt]
  {\footnotesize read}\\[1pt]
  {\scriptsize derived,\\[-1pt] never asserted}};
...
\node[ann, below=10pt] at (c.south) {Define rules\\before evidence};
\node[ann, below=10pt] at (n.south) {Narrate with\\five words};
\node[ann, below=10pt] at (l.south) {Publish\\threshold gates};
\node[ann, below=10pt] at (g.south) {Debit/credit\\must balance};
\node[ann, below=10pt] at (s.south) {Derived verdict\\never asserted};

%% --- Five-word vocabulary footer ---
\node[font=\scriptsize, text=black!55, text width=15cm, align=center]
  at (7.2, -2.8)
  {Canon vocabulary:\;
   \textbf{Drift}\,($\omega$)\enspace$\cdot$\enspace
   \textbf{Fidelity}\,($F$)\enspace$\cdot$\enspace
   \textbf{Roughness}\,($C$)\enspace$\cdot$\enspace
   \textbf{Return}\,($\tau_R$)\enspace$\cdot$\enspace
   \textbf{Integrity}\,($\IC$)};
...
\begin{axis}[
  ...
  ylabel={Kernel invariant value},
  xmin=1e-8, xmax=1,
  ymin=0, ymax=1.12,
  grid=both,
  grid style={gray!18, line width=0.2pt},
  major grid style={gray!32, line width=0.4pt},
  tick label style={font=\scriptsize},
  label style={font=\small},
  x dir=reverse,
  clip=false,
  legend style={font=\scriptsize, at={(0.97,0.62)},
    anchor=east, draw=gray!40, fill=white, fill opacity=0.92,
    text opacity=1, row sep=2pt, inner sep=4pt}]

%% Shaded floor zone
\addplot[fill=red!7, draw=none, forget plot]
  coordinates {(1e-8,0) (1e-8,0.114) (0.999,0.114) (0.999,0)} \closedcycle;

%% F line: arithmetic mean, stays healthy
\addplot[orange!70!black, dashed, line width=1.4pt,
  mark=none]
  coordinates {
  (0.999, 0.999)
  (0.5,   0.937)
...
\addlegendentry{$F$ (arithmetic mean — stays healthy)}

%% IC/F curve: collapses to 0.114
\addplot[blue!72!black, line width=1.7pt,
  mark=*, mark size=2.2pt,
  mark options={fill=blue!55!black, draw=blue!35!black}]
  coordinates {
  (0.999, 1.000)
  (0.5,   0.978)
  (0.1,   0.845)
...
\addlegendentry{$\IC/F$ (geometric/arithmetic ratio — collapses)}

%% Floor reference line
\addplot[red!62!black, dashed, line width=1.0pt]
  coordinates {(1e-8, 0.114) (0.999, 0.114)};
\addlegendentry{$\IC/F$ floor $= 0.114$ at $c_\mathrm{dead} = \eps$}

%% F healthy annotation — inside chart near middle
\node[font=\scriptsize, text=orange!72!black,
  fill=white, fill opacity=0.88, text opacity=1,
  inner sep=2.5pt, anchor=south]
  at (axis cs:1e-4, 0.895)
  {$F \to 0.874$\;(7 healthy channels preserve the mean)};

%% IC/F floor annotation — inside chart, below floor line
\node[font=\scriptsize, text=red!62!black,
  fill=white, fill opacity=0.88, text opacity=1,
  inner sep=2.5pt, anchor=north]
  at (axis cs:1e-4, 0.100)
  {$\IC/F \to 0.114$ as $c_\mathrm{dead} \to \eps = 10^{-8}$};

%% Gap annotation — text only, no arrows, in middle gap region
\node[font=\scriptsize, text=gray!58,
  fill=white, fill opacity=0.88, text opacity=1,
  inner sep=2.5pt, anchor=south west]
  at (axis cs:1e-3, 0.505)
  {gap $\Delta = F - \IC$ widens};

\end{axis}
\end{tikzpicture}
\caption{Geometric slaughter: one dead channel destroys
multiplicative coherence while leaving the arithmetic mean
nearly intact.  An 8-channel trace with 7~channels at
$c = 0.999$ and 1~channel at $c_\mathrm{dead}$.  As the dead
channel approaches $\eps = 10^{-8}$, $\IC/F$ (blue) drops
from $1.000$ to $0.114$, but $F$ (orange) falls only from
$0.999$ to $0.874$---a diverging heterogeneity gap
$\Delta = F - \IC$.  This mechanism underlies confinement
detection~\cite{wilson1974confinement} in the Standard Model
closure~\cite{pdg2024}: the color channel at $c \approx \eps$
in hadrons produces $\IC/F < 0.04$, making the
quark${\to}$hadron boundary visible as a $100{\times}$ drop
that $F$ alone would miss.}
\label{fig:slaughter}
\end{figure*}
...
\begin{axis}[
  ...
  legend style={font=\scriptsize, at={(0.03,0.03)},
    ...
    fill opacity=0.92, text opacity=1, inner sep=4pt}]

%% Background: red zone f < 0 (c < 0.5, collapse territory)
\addplot[fill=red!5, draw=none, forget plot]
  coordinates {(0.01,-3.0) (0.50,-3.0) (0.50,0.72) (0.01,0.72)} \closedcycle;

%% Background: green zone f > 0 (c > 0.5, return territory)
\addplot[fill=green!6, draw=none, forget plot]
  coordinates {(0.50,-3.0) (0.99,-3.0) (0.99,0.72) (0.50,0.72)} \closedcycle;

%% Equator divider
\draw[black!22, line width=0.5pt, dashed]
  (axis cs:0.5, -3.0) -- (axis cs:0.5, 0.72);

%% Vertical guides at landmark x-positions
\draw[brown!40, line width=0.5pt, densely dotted]
  (axis cs:0.318, -3.0) -- (axis cs:0.318, 0.72);
\draw[green!50!black!40, line width=0.5pt, densely dotted]
  (axis cs:0.782, -3.0) -- (axis cs:0.782, 0.72);

%% Main curve
\addplot[blue!72!black, smooth, samples=120,
  domain=0.02:0.98, line width=1.8pt]
  {(1-x)*ln(x/(1-x))};
\addlegendentry{$f(c) = (1{-}c)\ln\!\bigl[\tfrac{c}{1-c}\bigr]$\;
  (verified $<10^{-16}$ vs.\ closed form)}
...
%% Region labels
\node[font=\footnotesize, text=red!40!black, anchor=north, inner sep=2pt]
  at (axis cs:0.255, 0.67) {$f < 0$};
\node[font=\footnotesize, text=green!45!black, anchor=north, inner sep=2pt]
  at (axis cs:0.745, 0.67) {$f > 0$};

%% Equator: c = 1/2
\addplot[only marks, mark=*, mark size=3.0pt,
  mark options={fill=red!72!black, draw=red!45!black},
  forget plot]
  coordinates {(0.50, 0.0)};
\node[font=\scriptsize, text=red!72!black,
  fill=white, fill opacity=0.88, text opacity=1,
  inner sep=2.5pt, anchor=south west]
  at (axis cs:0.515, 0.04)
  {$c = \tfrac{1}{2}$:\enspace$f = 0$\enspace(equator)};

%% Maximum: c* = 0.7822
\addplot[only marks, mark=triangle*, mark size=3.5pt,
  mark options={fill=green!60!black, draw=green!35!black},
  forget plot]
  coordinates {(0.782, 0.279)};
\node[font=\scriptsize, text=green!60!black,
  fill=white, fill opacity=0.88, text opacity=1,
  inner sep=2.5pt, anchor=south]
  at (axis cs:0.782, 0.32)
  {$c^* = 0.7822$:\enspace$f_{\max}$};

%% Cardano root: c_trap = 0.3178
\addplot[only marks, mark=square*, mark size=2.5pt,
  mark options={fill=brown!75, draw=brown!50},
  forget plot]
  coordinates {(0.318, -0.521)};
\node[font=\scriptsize, text=brown!75!black,
  fill=white, fill opacity=0.88, text opacity=1,
  inner sep=2.5pt, anchor=north west]
  at (axis cs:0.338, -0.56)
  {$c_\mathrm{trap} = 0.318$\enspace($x^3+x-1=0$)};
...
the closed form $f(c) = (1{-}c)\ln[c/(1{-}c)]$.
Background shading marks the two regimes: $f < 0$ (left,
red, $\kappa$ dominates) and $f > 0$ (right, green, $S$
dominates).  Three structural landmarks: the \emph{equator}
$c = \tfrac{1}{2}$ where $f = 0$ exactly (quintuple fixed
point); the \emph{logistic self-dual} point $c^* = 0.7822$
where $f$ reaches its global maximum; and the
\emph{Cardano root} $c_\mathrm{trap} = 0.3178$ of
$x^3 + x - 1 = 0$ where $\Gam$ first drops below~1.
These three points form the skeleton of the Bernoulli
manifold.}
\label{fig:equator}
\end{figure*}
...
\begin{axis}[
  ...
  major grid style={gray!27, line width=0.35pt},
  axis equal image,
  clip=false]

%% Solvable region — blue tint below diagonal
\addplot[fill=blue!11, draw=none, forget plot]
  coordinates {(0,0) (1,1) (1,0)} \closedcycle;

%% Non-physical region — hatched above diagonal
\addplot[fill=red!7, draw=none,
  postaction={pattern=north east lines,
    pattern color=red!22}, forget plot]
  coordinates {(0,0) (1,1) (0,1)} \closedcycle;

%% Diagonal: IC = F
\addplot[blue!62!black, line width=1.4pt,
  forget plot]
  coordinates {(0,0) (1.02,1.02)};

%% Diagonal label
\node[font=\small, text=blue!60!black, rotate=45,
  fill=white, fill opacity=0.9, text opacity=1,
  inner sep=2.5pt]
  at (axis cs:0.74, 0.83)
  {$\IC = F$};

%% Region labels
\node[font=\footnotesize, text=blue!55!black]
  at (axis cs:0.75, 0.22) {\textit{Solvable}};
\node[font=\footnotesize, text=red!42!black, rotate=45]
  at (axis cs:0.22, 0.58) {\textit{Non-physical}};

%% Homogeneous point (near diagonal, high fidelity)
\addplot[only marks, mark=*, mark size=3.2pt,
  mark options={fill=blue!65!black}, forget plot]
  coordinates {(0.60, 0.58)};
\node[font=\scriptsize, anchor=south west, inner sep=2pt]
  at (axis cs:0.625, 0.58) {Homogeneous};

%% Heterogeneous point (far from diagonal, same F)
\addplot[only marks, mark=*, mark size=3.2pt,
  mark options={fill=red!65!black}, forget plot]
  coordinates {(0.60, 0.13)};
\node[font=\scriptsize, anchor=north west, inner sep=2pt]
  at (axis cs:0.625, 0.13) {Heterogeneous};

%% Gap arrow
\draw[{Stealth[length=4pt,width=3.5pt]}-{Stealth[length=4pt,width=3.5pt]},
  red!60!black, line width=0.8pt]
  (axis cs:0.54, 0.15) -- (axis cs:0.54, 0.56);
\node[font=\small, text=red!60!black,
  anchor=east, inner sep=2pt]
  at (axis cs:0.52, 0.36) {$\Delta$};

%% Recovery formula in bottom right
\node[font=\scriptsize, text=blue!52!black,
  fill=blue!4, draw=blue!20, rounded corners=2pt,
  anchor=south east, inner sep=3pt]
  at (axis cs:0.97, 0.02)
  {$c_{1,2} = F \pm \sqrt{F^2 - \IC^2}$};
...
\toprule
\rowcolor{blue!20}
{\color{white}\textbf{Lens}} &
{\color{white}\textbf{Drift}} &
{\color{white}\textbf{Fidelity}} &
{\color{white}\textbf{Roughness}} &
{\color{white}\textbf{Return}} &
...
\textbf{Epistemology} &
  ...
  Inference friction &
  Justified re-entry &
  \textit{\color{blue!50!black}(read from ledger)} \\
\textbf{Ontology} &
  State transition &
