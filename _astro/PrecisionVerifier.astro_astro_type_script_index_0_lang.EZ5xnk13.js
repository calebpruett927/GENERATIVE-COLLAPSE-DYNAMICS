import{c as f,v as F,a as E}from"./kernel.-GbphWvs.js";import"./constants.Dtlb4jqv.js";const y=document.getElementById("prec-run");y.addEventListener("click",()=>{const d=parseInt(document.getElementById("prec-n").value),r=parseInt(document.getElementById("prec-trials").value),p=document.getElementById("prec-results");p.innerHTML='<div class="bg-kernel-900 border border-kernel-600 rounded-lg p-4 text-center text-yellow-400 text-sm">Running...</div>',setTimeout(()=>{const $=performance.now();let l=0,a=0,i=0,m=0;const n={STABLE:0,WATCH:0,COLLAPSE:0};let c=0;const g=[];for(let t=0;t<r;t++){const s=Array.from({length:d},()=>Math.random()),x=Array.from({length:d},()=>1/d),e=f(s,x);F(e);const o=E(e),v=Math.abs(e.F+e.omega-1),b=Math.abs(e.IC-Math.exp(e.kappa)),u=e.IC<=e.F+1e-15;l=Math.max(l,v),a=Math.max(a,b),u||i++,m++,n[o.regime]++,o.isCritical&&c++,t<5&&g.push({c:s.slice(0,4),F:e.F,omega:e.omega,S:e.S,C:e.C,kappa:e.kappa,IC:e.IC,dualErr:v,expErr:b,icLeF:u,regime:o.regime+(o.isCritical?" (CRITICAL)":"")})}const h=performance.now()-$,k=i===0&&l<1e-10?"text-green-400":"text-red-400",C=i===0&&l<1e-10?"CONFORMANT":"NONCONFORMANT";p.innerHTML=`
        <!-- Verdict banner -->
