const c=document.querySelector('meta[name="base-url"]')?.getAttribute("content")||"/GENERATIVE-COLLAPSE-DYNAMICS/",g=[{sym:"F",name:"Fidelity",latin:"Fidelitas",formula:"F = Σ wᵢcᵢ",range:"[0, 1]",tier:1,desc:"What survives collapse — arithmetic mean of trace vector weighted by channel weights.",role:"Primitive"},{sym:"ω",name:"Drift",latin:"Derivatio",formula:"ω = 1 − F",range:"[0, 1]",tier:1,desc:"What is lost to collapse — measured departure from fidelity.",role:"Derived (from F)"},{sym:"S",name:"Bernoulli Field Entropy",latin:"Entropia",formula:"S = −Σ wᵢ[cᵢ ln cᵢ + (1−cᵢ) ln(1−cᵢ)]",range:"[0, ∞)",tier:1,desc:"Uncertainty of the collapse field. Shannon entropy is the degenerate limit.",role:"Primitive (computed, not free)"},{sym:"C",name:"Curvature",latin:"Curvatura",formula:"C = stddev(cᵢ) / 0.5",range:"[0, 1]",tier:1,desc:"Coupling to uncontrolled degrees of freedom — normalized standard deviation.",role:"Primitive (independent)"},{sym:"κ",name:"Log-Integrity",latin:"Log-Integritas",formula:"κ = Σ wᵢ ln(cᵢ,ε)",range:"(−∞, 0]",tier:1,desc:"Logarithmic sensitivity of coherence — one dead channel sends κ → −∞.",role:"Primitive"},{sym:"IC",name:"Composite Integrity",latin:"Integritas Composita",formula:"IC = exp(κ)",range:"(0, 1]",tier:1,desc:"Multiplicative coherence — the geometric mean. IC ≤ F always (integrity bound).",role:"Derived (from κ)"}],v=[{param:"ε",code:"EPSILON",value:"10⁻⁸",purpose:"Guard band — pole at ω=1 does not affect measurements to machine precision",source:"frozen_contract.EPSILON"},{param:"p",code:"P_EXPONENT",value:"3",purpose:"Unique integer where ω_trap is a Cardano root of x³ + x − 1 = 0",source:"frozen_contract.P_EXPONENT"},{param:"α",code:"ALPHA",value:"1.0",purpose:"Curvature cost coefficient (unit coupling) in D_C = α · C",source:"frozen_contract.ALPHA"},{param:"λ",code:"LAMBDA",value:"0.2",purpose:"Auxiliary coefficient",source:"frozen_contract.LAMBDA"},{param:"tol_seam",code:"TOL_SEAM",value:"0.005",purpose:"Seam residual tolerance: |s| ≤ tol for PASS — width where IC ≤ F holds at 100%",source:"frozen_contract.TOL_SEAM"},{param:"c*",code:"C_STAR",value:"0.7822",purpose:"Logistic self-dual fixed point: maximizes S + κ per channel",source:"constants"},{param:"c_trap",code:"OMEGA_TRAP",value:"0.3178",purpose:"Cardano root — the mirror of c* across equator (c_trap = 1 − c*)",source:"constants"}],x=[{regime:"Stable",condition:"ω < 0.038 AND F > 0.90 AND S < 0.15 AND C < 0.14",share:"12.5%",color:"#22c55e",note:"Conjunctive — ALL gates must be satisfied. Stability is rare."},{regime:"Watch",condition:"0.038 ≤ ω < 0.30 (or Stable gates not all met)",share:"24.4%",color:"#f59e0b",note:"Intermediate zone — some drift but not yet collapsed."},{regime:"Collapse",condition:"ω ≥ 0.30",share:"63.1%",color:"#ef4444",note:"Most of the manifold. Return from collapse is what the axiom measures."},{regime:"Critical (overlay)",condition:"IC < 0.30",share:"—",color:"#a855f7",note:"Severity overlay — accompanies any regime. Flags dangerously low integrity."}],y=[{lens:"Epistemology",drift:"Change in belief/evidence",fidelity:"Retained warrant",roughness:"Inference friction",return:"Justified re-entry",color:"#f59e0b"},{lens:"Ontology",drift:"State transition",fidelity:"Conserved properties",roughness:"Heterogeneity / seams",return:"Restored coherence",color:"#3b82f6"},{lens:"Phenomenology",drift:"Perceived shift",fidelity:"Stable features",roughness:"Distress / effort",return:"Coping that holds",color:"#a855f7"},{lens:"History",drift:"Periodization",fidelity:"What endures",roughness:"Rupture / confound",return:"Restitution",color:"#f43f5e"},{lens:"Policy",drift:"Regime shift",fidelity:"Mandate persistence",roughness:"Friction / cost",return:"Reinstatement",color:"#10b981"},{lens:"Semiotics",drift:"Sign departure",fidelity:"Convention survived",roughness:"Meaning loss",return:"Interpretant closure",color:"#06b6d4"}],a=[{stop:"Contract",role:"Define before evidence",desc:"Freeze sources, normalization, near-wall policy, thresholds. Declares the rules before the sentence is written.",icon:"📜"},{stop:"Canon",role:"Narrate with 5 words",desc:"Tell the story using Drift, Fidelity, Roughness, Return, Integrity. Prose-first, auditable by construction.",icon:"📖"},{stop:"Closures",role:"Publish thresholds",desc:"No mid-episode edits. Version the sheet. Stance must change when thresholds are crossed.",icon:"🔒"},{stop:"Integrity Ledger",role:"Debit/credit reconcile",desc:"Debit Drift + Roughness, credit Return. The account must reconcile: residual ≤ tol.",icon:"📊"},{stop:"Stance",role:"Derived verdict",desc:"Read from declared gates: Stable / Watch / Collapse (+ Critical overlay). Never asserted, always derived.",icon:"⚖️"}],b=[{wrong:"Shannon entropy",right:"Bernoulli field entropy",reason:"Shannon is the degenerate limit when the collapse field is removed"},{wrong:"AM-GM inequality",right:"Integrity bound (IC ≤ F)",reason:"Derived independently from Axiom-0; AM-GM is degenerate limit"},{wrong:"AM-GM gap",right:"Heterogeneity gap (Δ = F − IC)",reason:"Measures channel heterogeneity, not an inequality"},{wrong:"rederives / recovers",right:"Derives independently",reason:"Arrow runs from axiom to classical, not reverse"},{wrong:"unitarity",right:"Duality identity F + ω = 1",reason:"Structural identity of collapse, not quantum unitarity"},{wrong:"hyperparameter",right:"Frozen parameter",reason:"Seam-derived, not tuned — trans suturam congelatum"},{wrong:"constant (for frozen params)",right:"Frozen / consistent across the seam",reason:"Discovered by seam, not chosen by convention"}],p={gcd:["E1","E2","E3","E4","B1","B5","D1","D3"],rcft:["E1","E2","E3","B5","B6","D5","D8"],kinematics:["E1","E3","B1","B3","D1"],weyl:["E1","E3","B1","B7","D4"],standard_model:["E1","E2","E3","E4","B1","B3","B5","D3","D5"],nuclear_physics:["E1","E3","B3","B5","D3","D5"],quantum_mechanics:["E1","E2","E3","B1","B5","D1","D3"],atomic_physics:["E1","E2","E3","B1","B3","D3","D5"],materials_science:["E1","E3","B1","B3","D5"],everyday_physics:["E1","E3","B1","D1"],finance:["E1","E3","B1","B5","D1"],security:["E1","E3","B1"],astronomy:["E1","E3","B1","B7","D4"],evolution:["E1","E3","B1","B3","D1","D5"],consciousness_coherence:["E1","E3","B1","B5","D1"],awareness_cognition:["E1","E3","B1","D1","D5"],clinical_neuroscience:["E1","E3","B1","B5","D3"],dynamic_semiotics:["E1","E3","B1","B5","D1"],continuity_theory:["E1","E3","B1","D1","D8"],spacetime_memory:["E1","E3","B1","B7","D4","D5"]},f={gcd:"GCD",rcft:"RCFT",kinematics:"Kinematics",weyl:"WEYL",standard_model:"Std Model",nuclear_physics:"Nuclear",quantum_mechanics:"QM",atomic_physics:"Atomic",materials_science:"Materials",everyday_physics:"Everyday",finance:"Finance",security:"Security",astronomy:"Astronomy",evolution:"Evolution",consciousness_coherence:"Consciousness",awareness_cognition:"Awareness",clinical_neuroscience:"Clinical",dynamic_semiotics:"Semiotics",continuity_theory:"Continuity",spacetime_memory:"Spacetime"};let d="symbols",n="";function k(){return`<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      `).join("")}
    </div>`}function E(){return`<div class="space-y-3">
      ${v.filter(e=>!n||`${e.param} ${e.code} ${e.purpose}`.toLowerCase().includes(n)).map(e=>`
    ...
    </div>`}function $(){return`<div class="space-y-4">
      ${x.map(t=>`
      ...
      ${y.map(t=>`
        <div class="bg-kernel-900 border border-kernel-700 rounded-lg p-4">
          <div class="flex items-center gap-2 mb-3">
            ...
            <a href="${c}rosetta/" class="ml-auto text-[10px] text-kernel-500 hover:text-kernel-300">Open Rosetta →</a>
          </div>
      ...
      `).join("")}
    </div>`}function w(){return`<div class="space-y-1">
      <div class="flex items-center justify-center gap-1 text-xs font-mono text-kernel-500 mb-6 flex-wrap">
        ${a.map((t,e)=>`
          ...
          ${e<a.length-1?'<span class="text-kernel-600">→</span>':""}
        `).join("")}
      </div>
      ${a.map((t,e)=>`
      ...
      `).join("")}
    </div>`}function C(){return`<div class="space-y-2">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-xs">
          ...
          <tbody>
            ${b.filter(e=>!n||`${e.wrong} ${e.right} ${e.reason}`.toLowerCase().includes(n)).map(e=>`
    ...
    </div>`}function B(){const t=["E1","E2","E3","E4","B1","B3","B5","B6","B7","D1","D3","D4","D5","D8"],e=Object.keys(p),o=document.getElementById("cross-matrix");if(!o)return;let r='<thead><tr><th class="py-1 px-2 text-left text-kernel-500 sticky left-0 bg-kernel-950">Domain</th>';for(const s of t)r+=`<th class="py-1 px-2 text-center text-kernel-500 font-mono">${s}</th>`;r+="</tr></thead><tbody>";for(const s of e){const h=p[s]||[];r+='<tr class="border-b border-kernel-800/50 hover:bg-kernel-900/50">',r+=`<td class="py-1.5 px-2 sticky left-0 bg-kernel-950"><a href="${c}${s}/" class="text-kernel-300 hover:text-amber-400 no-underline">${f[s]}</a></td>`;for(const u of t)h.includes(u)?r+=`<td class="py-1 px-2 text-center"><a href="${c}identities/" class="inline-block w-3 h-3 rounded-full bg-amber-500/70 hover:bg-amber-400 transition no-underline" title="${f[s]} × ${u}"></a></td>`:r+='<td class="py-1 px-2 text-center"><span class="inline-block w-3 h-3 rounded-full bg-kernel-800/50"></span></td>';r+="</tr>"}r+="</tbody>",o.innerHTML=r}const i={constitution:[{name:"Reditus",kind:"object",desc:"The admissible return of structure through collapse under declared conditions."},{name:"Structura Reditus",kind:"field",desc:"The field that studies admissible return."},{name:"GCD",kind:"theory",desc:"Generative Collapse Dynamics — the foundational theory that generates both axes."}],authority:[{tier:"Tier-1",name:"The Kernel",desc:"The kernel function and its provable identities. Immutable within a run."},{tier:"Tier-0",name:"Protocol",desc:"Embedding, regime gates, seam calculus, validation — the code that computes the kernel."},{tier:"Tier-2",name:"Expansion Space",desc:"Domain closures: which real-world quantities become channels. Freely extensible."}],functional:[{sys:"UMCP",verb:"measures",desc:"Universal Measurement Contract Protocol — validates reproducible workflows against contracts."},{sys:"RCFT",verb:"explores",desc:"Recursive Collapse Field Theory — the discovery engine; develops Tier-2 closures."},{sys:"ULRC",verb:"preserves language",desc:"Unified Language of Recursive Collapse — translates meaning across tiers without drift."}]};function S(){const t=(e,o,r)=>`
      <div>
        <div class="text-[10px] uppercase tracking-wider mb-2" style="color:${o}">${e}</div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          ${r.map(s=>`
            <div class="bg-kernel-900 border border-kernel-700 rounded-lg p-4">
              <div class="flex items-baseline gap-2 mb-1">
                <span class="text-sm font-bold text-kernel-200">${s.a}</span>
                <span class="text-[10px] uppercase text-kernel-600">${s.b}</span>
              </div>
              <div class="text-xs text-kernel-400">${s.desc}</div>
            </div>
          `).join("")}
        </div>
      </div>`;return`<div class="space-y-6">
      <p class="text-xs text-kernel-500">
        GCD generates two orthogonal axes (separation of powers). The <strong class="text-kernel-300">authority axis</strong>
        (tiers) governs what may change; the <strong class="text-kernel-300">functional axis</strong> (systems) governs
        what is done. A functional system operates across all tiers but is never a tier.
      </p>
      ${t("The Constitution — object before field, field before theory","#fbbf24",i.constitution.map(e=>({a:e.name,b:e.kind,desc:e.desc})))}
      ${t("Authority Axis — what may be changed (tiers)","#60a5fa",i.authority.map(e=>({a:e.tier,b:e.name,desc:e.desc})))}
      ${t("Functional Axis — what is being done (systems)","#4ade80",i.functional.map(e=>({a:e.sys,b:e.verb,desc:e.desc})))}
      <div class="text-xs text-kernel-500 px-3 py-2 bg-kernel-900/50 rounded border border-kernel-800 italic">
        The two axes are orthogonal: tiers govern authority, systems govern operation. The systems operate across the tiers; the tiers constrain what each system may alter.
      </div>
    </div>`}const A={symbols:k,frozen:E,regime:$,lenses:D,spine:w,terminology:C,architecture:S};function m(){const t=document.getElementById("ref-content");if(!t)return;const e=A[d];t.innerHTML=e?e():""}document.querySelectorAll(".ref-tab-btn").forEach(t=>t.addEventListener("click",()=>{d=t.dataset.tab,document.querySelectorAll(".ref-tab-btn").forEach(e=>{e.dataset.tab===d?(e.classList.remove("bg-kernel-800","text-kernel-400"),e.classList.add("bg-kernel-700","text-kernel-200")):(e.classList.remove("bg-kernel-700","text-kernel-200"),e.classList.add("bg-kernel-800","text-kernel-400"))}),m()}));const l=document.getElementById("ref-search");l&&l.addEventListener("input",()=>{n=l.value.toLowerCase().trim(),m()});m();B();
