...
**UMCP v2.3.0** · **16,204 tests** · **20 domains** · **193 closure modules** · **47 lemmas** · **44 structural identities**
...
pytest                                       # 16,204 tests (pytest --collect-only | grep ":" | wc -l to verify)
...
**16,204 test cases** across **187 test files** in `tests/` (186 top-level `test_*.py` + 1 in `tests/closures/` + `conftest.py`), numbered by tier and domain (`test_000_*` through `test_293_*`). Single `tests/conftest.py` provides:
...
- Parametrized tests expand the collected items to 16,204 (verify: `pytest --collect-only | grep "::" | wc -l`)
