...
**UMCP v2.2.5** · **14,565 tests** · **20 domains** · **181 closure modules** · **47 lemmas** · **44 structural identities**
...
pytest                                       # 14,565 tests (pytest --collect-only | grep ":" | wc -l to verify)
...
**14,565 test cases** across **172 test files** in `tests/` (171 top-level `test_*.py` + 1 in `tests/closures/` + `conftest.py`), numbered by tier and domain (`test_000_*` through `test_293_*`). Single `tests/conftest.py` provides:
...
- Parametrized tests expand the collected items to 14,565 (verify: `pytest --collect-only | grep "::" | wc -l`)
