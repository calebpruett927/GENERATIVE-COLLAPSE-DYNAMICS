...
pytest                                       # 15,260 tests (pytest --collect-only | grep ":" | wc -l to verify)
...
**15,260 test cases** across **178 test files** in `tests/` (177 top-level `test_*.py` + 1 in `tests/closures/` + `conftest.py`), numbered by tier and domain (`test_000_*` through `test_293_*`). Single `tests/conftest.py` provides:
...
- Parametrized tests expand the collected items to 15,260 (verify: `pytest --collect-only | grep "::" | wc -l`)
