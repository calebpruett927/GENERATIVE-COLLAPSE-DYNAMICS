...
class UMCPContractFormatter:
    ...
    REQUIRED_TIERS = {
        "UMA": {
            "tier_level": 0,
            "required_fields": [
                "id", "version", "timezone", "embedding", 
                "tier_1_kernel", "typed_censoring"
            ],
        ...
        "GCD": {
            "tier_level": 1,
            "required_fields": [
                "id", "version", "parent_contract", "tier_level",
                "axioms", "regime_classification", "mathematical_identities"
            ],
            "axiom_check": "AX-0",
            "parent": "UMA"
        },
    ...
    def format_contract(self, contract_path: Path, output_path: Optional[Path] = None) -> bool:
        """
    ...
    def _detect_tier(self, contract: Dict[str, Any]) -> Optional[str]:
    ...
    def _validate_tier_requirements(self, contract: Dict[str, Any], tier: str) -> bool:
        ...
        if tier == "UMA":
            ...
            axiom_found = any(
                a.get("id") == axiom_key for a in axioms
            )
            if not axiom_found:
...
def main():
    ...
    parser.add_argument(
        "contract",
        nargs="?",
        help="Contract file to format"
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Format all contracts in repository"
    )
    parser.add_argument(
        "--validate",
        action="store_true",
        help="Validate without formatting"
    )
    parser.add_argument(
        "--strict",
        action="store_true",
