...
def compute_stellar_luminosity_array(
    ...
    return [
        compute_stellar_luminosity(m, t, r)
        for m, t, r in zip(masses, temperatures, radii, strict=True)
    ]
...
if __name__ == "__main__":
    ...
    print(f"Sun:    L_pred={result['L_predicted']:.4f}  L_SB={result['L_SB']:.4f}"
    ...
    print(f"Sirius: L_pred={result['L_predicted']:.4f}  L_SB={result['L_SB']:.4f}"
          f"  delta_L={result['delta_L']:.4f}  λ_peak={result['lambda_peak']:.1f} nm"
          f"  regime={result['regime']}")

    # Betelgeuse: M≈18, T≈3600, R≈764
    result = compute_stellar_luminosity(18.0, 3600.0, 764.0)
    print(f"Betel:  L_pred={result['L_predicted']:.4f}  L_SB={result['L_SB']:.4f}"
          f"  delta_L={result['delta_L']:.4f}  λ_peak={result['lambda_peak']:.1f} nm"
          f"  regime={result['regime']}")
