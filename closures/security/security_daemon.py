...
class SecurityDaemon:
    ...
    def __init__(
        ...
        self.logger.info(f"Axiom: 'What Survives Validation Is Trusted'")
    ...
    def _validate_file(self, file_path: str):
        ...
        result = self.validator.validate_signals(
            signals,
            signal_history=signal_history if len(signal_history) > 1 else None,
            entity_id=entity_id
        )
        ...
        if len(entity.validation_history) > 1:
            ...
            seam = compute_trust_seam(
                prev.invariants.sigma,
                result.invariants.sigma,
                prev.invariants.TIC,
                result.invariants.TIC
            )
            entity.trust_seam_ledger.append(
                entity.trust_seam_ledger[-1] + seam["delta_sigma_ledger"]
            )
    ...
    def _revalidate_entity(self, entity: MonitoredEntity):
        ...
        if result.status != entity.current_status:
            self.logger.info(
                f"Status change for {entity.entity_id}: "
                f"{entity.current_status} → {result.status}"
            )
...
def main():
    ...
    parser.add_argument(
        '--log-dir',
        default='/tmp/umcp_security/logs',
        help='Log directory'
    )
    parser.add_argument(
        '--state-dir',
        default='/tmp/umcp_security/state',
