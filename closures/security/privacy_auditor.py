...
def audit_data_privacy(
    data: Dict[str, Any],
    privacy_rules: Optional[Dict[str, Any]] = None,
    consent_records: Optional[Dict[str, bool]] = None,
    ...
    if consent_records is not None:
        ...
        for pii in all_pii:
            if pii.pii_type in require_consent:
                if not consent_records.get(pii.pii_type, False):
...
def generate_privacy_report(
    ...
    return {
        ...
        "pii_summary": {
            "total_found": len(audit_result.pii_found),
            "by_type": pii_by_type
        },
        "violations_summary": {
            "total": len(audit_result.violations),
...
if __name__ == "__main__":
    ...
    result = audit_data_privacy(
        data=test_data,
        consent_records={"email": True, "ssn": False},
        data_age_days=400,
        is_encrypted=False
    )
