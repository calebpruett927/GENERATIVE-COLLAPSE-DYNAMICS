...
class ResponseEngine:
    ...
    def __init__(
        self,
        quarantine_dir: str = "/tmp/umcp_quarantine",
        dry_run: bool = False
    ):
        """
    ...
    def _allow(self, decision: ResponseDecision) -> bool:
        ...
        self.logger.info(
            f"ALLOW: {decision.entity_id} "
            f"(T={decision.T:.3f}, τ_A={decision.tau_A})"
        )
        return True
    ...
    def _allow_monitored(self, decision: ResponseDecision) -> bool:
        ...
        self.logger.warning(
            f"ALLOW_MONITORED: {decision.entity_id} "
            f"(T={decision.T:.3f}, τ_A={decision.tau_A}) - {decision.reason}"
        )
    ...
    def _quarantine(self, decision: ResponseDecision) -> bool:
        """Quarantine entity."""
        self.logger.warning(
            f"QUARANTINE: {decision.entity_id} "
            f"(T={decision.T:.3f}, τ_A={decision.tau_A})"
        )
    ...
    def _terminate(self, decision: ResponseDecision) -> bool:
        """Terminate process."""
        self.logger.error(
            f"TERMINATE: {decision.entity_id} "
            f"(T={decision.T:.3f}, τ_A={decision.tau_A})"
        )
