...
def classify_trust_status(
    T: float,
    tau_A: Optional[Union[int, str]],
    ...
    # Low trust = blocked
    if T < thresholds["suspicious"]:
        return "BLOCKED"
    
    # High trust with timely return = trusted
    if T >= thresholds["trusted"] and (tau_A is None or (isinstance(tau_A, (int, float)) and tau_A <= thresholds["max_tau_A"])):
        return "TRUSTED"
