...
def compute_phase_anchor_return_hysteresis(
    x: np.ndarray, 
    v: np.ndarray, 
    t: np.ndarray,
    h: float = 0.05
...
def compute_IC_smooth_contribution(
    ...
    ic_result = np.ones(N)
    for c_i, w_i in zip(contributions, channel_weights):
...
def generate_shm_run() -> None:
    ...
    IC_pass_rate = float(np.mean(IC >= IC_min))
    
    kernel_df = pd.DataFrame({
        "t": t, "omega": omega, "F": fidelity_arr, "S": S_norm, "C": curvature_arr,
        "tau_R": tau_R, "IC": IC, "kappa": kappa, "kappa_cum": kappa_cum,
    })
    ...
    oor_df = pd.DataFrame({
        "t": t, "channel": ["x"] * N, "oor": oor_mask.astype(int), "rate": oor_mask.astype(float),
    })
    ...
    artifacts.append({"path": "kernel/kernel.csv", "sha256": kernel_hash, "role": "kernel", "bytes": len(kernel_df.to_csv())})
    ...
    artifacts.append({"path": "tables/tauR_series.csv", "sha256": tauR_hash, "role": "table", "bytes": len(tauR_df.to_csv())})
    ...
    artifacts.append({"path": "logs/censor.csv", "sha256": censor_hash, "role": "log", "bytes": len(censor_df.to_csv())})
    ...
    frozen = {
        ...
        "adapter": {
            "name": "SHM.ADAPTER", "dt": dt, "T": T, "f0": f0, 
            "omega_0": omega_0, "A": A, "noise_sigma": noise_sigma
        },
    ...
    artifacts.append({"path": "config/frozen.json", "sha256": frozen_hash, "role": "config", "bytes": len(json.dumps(frozen))})
    ...
    print(f"  TIER-1 CHECK: F = 1 - ω ? {identity_validation['F_identity_valid']} (max error: {identity_validation['F_max_error']:.2e})")
    ...
    print(f"  TIER-1 CHECK: κ = ln(IC) ? {identity_validation['kappa_identity_valid']} (max error: {identity_validation['kappa_max_error']:.2e})")
...
def generate_ballistic_run() -> None:
    ...
    kernel_df = pd.DataFrame({
        "t": t_arr, "omega": omega, "F": fidelity_arr, "S": S_norm, "C": curvature_arr,
        "tau_R": tau_R, "IC": IC, "kappa": kappa, "kappa_cum": kappa_cum,
    })
    ...
    oor_df = pd.DataFrame({"t": t_arr, "channel": ["y"] * N, "oor": oor_mask.astype(int), "rate": oor_mask.astype(float)})
    
    censor_mask = ~np.isfinite(tau_R)
    censor_df = pd.DataFrame({"t": t_arr, "channel": ["tau_R"] * N, "censored": censor_mask.astype(int), "rate": censor_mask.astype(float)})
    ...
    artifacts = []
    artifacts.append({"path": "kernel/kernel.csv", "sha256": write_csv(run_dir / "kernel" / "kernel.csv", kernel_df), "role": "kernel", "bytes": len(kernel_df.to_csv())})
    artifacts.append({"path": "derived/psi.csv", "sha256": write_csv(run_dir / "derived" / "psi.csv", psi_df), "role": "derived", "bytes": len(psi_df.to_csv())})
    artifacts.append({"path": "tables/tauR_series.csv", "sha256": write_csv(run_dir / "tables" / "tauR_series.csv", tauR_df), "role": "table", "bytes": len(tauR_df.to_csv())})
    artifacts.append({"path": "logs/oor.csv", "sha256": write_csv(run_dir / "logs" / "oor.csv", oor_df), "role": "log", "bytes": len(oor_df.to_csv())})
    artifacts.append({"path": "logs/censor.csv", "sha256": write_csv(run_dir / "logs" / "censor.csv", censor_df), "role": "log", "bytes": len(censor_df.to_csv())})
    artifacts.append({"path": "seams/seam_ledger.csv", "sha256": write_csv(run_dir / "seams" / "seam_ledger.csv", seam_df), "role": "seam", "bytes": len(seam_df.to_csv())})
    artifacts.append({"path": "receipts/weld_receipt.json", "sha256": write_json(run_dir / "receipts" / "weld_receipt.json", weld_receipt), "role": "receipt", "bytes": len(json.dumps(weld_receipt))})
    ...
    frozen = {
        ...
        "adapter": {
            "name": "BALLISTIC.ADAPTER", "dt": dt, "g": g, "v0": v0, 
            "theta_deg": float(np.degrees(theta)), "restitution": restitution
        },
        "return": {"format": "csv", "columns": ["t", "omega", "F", "S", "C", "tau_R", "IC", "kappa", "kappa_cum"]},
        "tier1_identities": {
            "omega_definition": "||Ψ(t) - Ψ(t-Δt)|| / max, normalized to [0,1]",
    ...
    print(f"  τ_R: ALWAYS CENSORED (non-recurrent by design)")
...
def generate_gait_run() -> None:
    ...
    censor_mask = ~np.isfinite(tau_R)
    censor_df = pd.DataFrame({"t": t, "channel": ["tau_R"] * N, "censored": censor_mask.astype(int), "rate": censor_mask.astype(float)})
    ...
    frozen = {
        ...
        "adapter": {
            "name": "GAIT.ADAPTER", "fs": fs, "T": T, "f_gait": f_gait,
