...
def compute_phase_anchor_return(
    x: np.ndarray, 
    v: np.ndarray, 
    t: np.ndarray,
    dt: float
...
def generate_shm_run() -> None:
    ...
    frozen = {
        ...
        "adapter": {"name": "SHM.ADAPTER", "dt": dt, "T": T, "f0": f0, "omega_0": omega_0, "A": A, "noise_sigma": noise_sigma},
    ...
    print(f"  τ_R: coverage={100*len(tau_R_finite)/N:.1f}%, median={np.median(tau_R_finite):.3f}s (target≈{expected_period:.3f}s)")
...
def generate_gait_run() -> None:
    ...
    kernel_df = pd.DataFrame({
        "t": t, "omega": omega_drift, "F": fidelity_arr, "S": S, "C": curvature_arr,
        "tau_R": tau_R, "IC": IC, "kappa": kappa,
    })
