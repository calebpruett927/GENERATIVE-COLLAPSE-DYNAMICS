...
def generate_shm_run() -> None:
    ...
    kernel_df = pd.DataFrame({
        "t": t, "omega": omega, "F": fidelity_arr, "S": S, "C": curvature_arr,
        "tau_R": tau_R, "IC": IC, "kappa": kappa,
    })
    ...
    frozen = {
        ...
        "adapter": {
            "name": "SHM.ADAPTER",
            "dt": dt, "T": T, "f0": f0, "omega_0": omega_0, "A": A, "noise_sigma": noise_sigma,
        },
    ...
    print(f"  τ_R: coverage={100*len(tau_R_finite)/N:.1f}%, median={np.median(tau_R_finite):.3f}s, expected≈{1/f0:.3f}s")
...
def generate_ballistic_run() -> None:
    ...
    kernel_df = pd.DataFrame({
        "t": t_arr, "omega": omega, "F": fidelity_arr, "S": S, "C": curvature_arr,
        "tau_R": tau_R, "IC": IC, "kappa": kappa,
    })
    ...
    frozen = {
        ...
        "adapter": {"name": "BALLISTIC.ADAPTER", "dt": dt, "g": g, "v0": v0, "theta_deg": float(np.degrees(theta)), "restitution": restitution},
        "return": {"format": "csv", "columns": ["t", "omega", "F", "S", "C", "tau_R", "IC", "kappa"]},
        "weights": {"omega": 1.0, "F": 1.0, "S": 1.0, "C": 1.0, "tau_R": 1.0},
        "closures": ["gamma.default.v1", "norms.l2_eta1e-3.v1"],
    }
    artifacts.append({"path": "config/frozen.json", "sha256": write_json(run_dir / "config" / "frozen.json", frozen), "role": "config", "bytes": len(json.dumps(frozen))})
    
    write_json(run_dir / "manifest.json", {
        "casepack_id": "KIN.CP.BALLISTIC", "run_id": run_id,
...
def generate_gait_run() -> None:
    ...
    frozen = {
        ...
        "adapter": {"name": "GAIT.ADAPTER", "fs": fs, "T": T, "f_gait": f_gait, "channels": ["ax", "ay", "az", "gx", "gy", "gz"]},
        "return": {"format": "csv", "columns": ["t", "omega", "F", "S", "C", "tau_R", "IC", "kappa"]},
    ...
    write_json(run_dir / "manifest.json", {
        "casepack_id": "KIN.CP.GAIT", "run_id": run_id,
        "created_utc": datetime.now(timezone.utc).isoformat(), "artifacts": artifacts,
    })
    ...
    print(f"  τ_R: coverage={100*len(tau_R_finite)/N:.1f}%, median={np.median(tau_R_finite):.3f}s, expected≈{expected_period:.3f}s")
    print(f"  F: mean={np.mean(fidelity_arr):.3f}, IC pass rate={100*np.mean(IC):.1f}%")
    print(f"  κ final: {kappa[-1]:.4f}")
