...
class UMCPKinematics:
    ...
    def bounded_embed(self, q: float, q_ref: float, channel: str, t: int) -> float:
        ...
        if raw > 1e308:
            self.oor_flags.append({
                'channel': channel, 't': t, 'type': 'OVERFLOW', 'raw': 'EXCEEDED_FLOAT64'
            })
            return 1.0 - FrozenParams.EPSILON
        
        if raw > 1.0:
            self.oor_flags.append({
                'channel': channel, 't': t, 'type': 'CLIPPED', 'raw': raw
            })
            return 1.0
    ...
    def compute_phase_distance_squared(
        self, gamma1: Tuple[float, float], gamma2: Tuple[float, float]
    ) -> float:
    ...
    def compute_kinematic_return(
        ...
        if domain_size == 0:
            return (
                KinSpecialValue.UNIDENTIFIABLE_KIN,
                0.0,
                ReturnClassification.NON_RETURNING
            )
        
        # Normalize series
        x_tilde = [self.bounded_embed(x, FrozenParams.L_REF, 'x', i) 
                   for i, x in enumerate(x_series)]
        v_tilde = [self.bounded_embed(v, FrozenParams.v_ref, 'v', i) 
    ...
    def compute_kinematic_credit(
        self, tau_kin: TauKin, return_rate: float, t: int
    ) -> float:
    ...
    def verify_momentum_conservation(
        self, p_series: List[float], F_ext_series: List[float]
    ) -> Dict[str, Any]:
        """KIN-AX-2 compliant with precondition checking"""
        
        if len(p_series) < 2:
            return {
                'status': ConservationStatus.INSUFFICIENT_DATA,
    ...
    def verify_energy_conservation(
        self, E_series: List[float], W_nc_series: List[float]
    ) -> Dict[str, Any]:
        """KIN-AX-2 compliant with precondition checking"""
...
def test_conservation_with_preconditions():
    ...
    print(f"  UMCP result:     status={umcp_result['status'].value}, precondition_met={umcp_result['precondition_met']}")
    print(f"  ")
    print(f"  Standard says: VIOLATED (FALSE ALARM!)")
    print(f"  UMCP says:     NOT_APPLICABLE (correct - precondition F_ext=0 not met)")
    ...
    print(f"  UMCP result:     status={umcp_result['status'].value}, precondition_met={umcp_result['precondition_met']}")
    print(f"  Both correctly identify CONSERVED ✓")
    ...
    print(f"  UMCP result:     status={umcp_result['status'].value}, precondition_met={umcp_result['precondition_met']}")
    print(f"  ")
    print(f"  Standard says: VIOLATED")
    print(f"  UMCP says:     NOT_APPLICABLE (W_nc ≠ 0, friction is doing work)")
...
def test_periodicity_detection():
    ...
    for name, signal_fn, noise_std in scenarios:
        ...
        # UMCP phase return
        _, return_rate, classification = umcp.compute_kinematic_return(
            x_list, v_list, 80
        )
...
def test_typed_infinity():
    ...
    print(f"  Cannot distinguish escape from overflow!")
    ...
    print(f"  Types are distinct and comparable!")
    ...
    tau_kin, rate, classification = umcp.compute_kinematic_return(
        list(x_drift), list(v_drift), t_idx
    )
    
    print(f"  Drifting trajectory:")
...
def test_noise_robustness():
    ...
    for noise_std in noise_levels:
        ...
        # UMCP
        tau_kin, return_rate, _ = umcp.compute_kinematic_return(
            list(x_noisy), list(v_noisy), 80
        )
...
def test_discontinuity_detection():
    ...
    for jt, jm in zip(jump_times, jump_magnitudes):
        x_with_jumps[jt:] += jm
...
def test_multi_trajectory_comparison():
    ...
    for x0, v0, a, name in particles:
        ...
        tau_kin, _, classification = umcp.compute_kinematic_return(
            x_series, v_series, n_steps - 1
        )
...
def test_phase_space_coverage():
    ...
    for name, x_arr, v_arr in trajectories:
        ...
        unique_cells = len(set(zip(x_bins, v_bins)))
