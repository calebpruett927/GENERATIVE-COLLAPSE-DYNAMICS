...
def generate_domain_markdown(
    ...
    sections.append("---\n")
    sections.append(
        f"*Generated by the Headless Contract Gateway (HCG) · Domain: {data.domain} · Lens: {lens} · UMCP v2.3.1*\n"
    )
...
def generate_index_markdown(domains: list[SiteData]) -> str:
    ...
    lines.append("")
    lines.append("---\n")
    lines.append("*Generated by the Headless Contract Gateway (HCG) · UMCP v2.3.1*\n")
    return "\n".join(lines)
