...
KERNEL_SYMBOLS = {
    ...
    "kappa": "κ (Curvature)",
    "stiffness": "Stiffness",
    "delta_kappa": "Δκ (Delta Curvature)",
    "curvature": "Curvature",
    "freshness": "Freshness (1-ω)",
    "seam_residual": "Seam Residual (s)",
    "timestamp": "Timestamp",
    "run_status": "Status",
}
...
def render_health_page() -> None:
    ...
    with col2:
        if st.button("🧪 Run Tests (Quick)", use_container_width=True):
...
def main() -> None:
    ...
    df = load_ledger()
    st.sidebar.caption(f"📒 {len(df)} ledger entries")
    st.sidebar.caption(f"📦 {len(load_casepacks())} casepacks")
    st.sidebar.caption(f"📜 {len(load_contracts())} contracts")
    ...
    st.sidebar.markdown("- [API](http://localhost:8000/docs)")
