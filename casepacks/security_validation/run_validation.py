...
from closures.security.trust_fidelity import classify_trust_status, compute_trust_fidelity
from closures.security.trust_integrity import compute_trust_integrity


def load_security_signals(path: str | Path) -> np.ndarray:
...
def run_casepack_validation():
    ...
    tau_A_results = compute_anomaly_return_series(signal_series, eta=0.01, horizon=64, weights=weights)
    ...
    for inv in invariants_list:
        ...
        print(
            f"{inv['t']:>3} {inv['T']:>6.3f} {inv['theta']:>6.3f} "
    ...
    for event in anomaly_events:
        print(f"  - {event['type']}: t={event['start']} to t={event['end']} (duration={event.get('duration', '?')})")
    ...
    if final_inv["tau_A"] is not None and final_inv["tau_A"] != "INF_ANOMALY":
        ...
        print("  ✓ Trust earned through validated return")
    else:
        print(f"  ✗ No return to baseline (τ_A = {final_inv['tau_A']})")
        print("  ✗ Trust NOT earned - entity remains non-canonical")
