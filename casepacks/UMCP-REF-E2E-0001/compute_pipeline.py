...
def stage_export(kernel_rows: List[Dict], regime_rows: List[Dict]):
    ...
    # Count OOR events
    oor_count = 0
    with open(PSI_CSV, "r", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
    ...
    receipt = {
        ...
        "receipt": {
            ...
            "regime_summary": {
                ...
                "final_regime": regime_rows[-1]["regime"]
            },
            "typed_boundaries": {
                "oor_count": oor_count,
...
def main():
    ...
    log.log(f"Duration: {(datetime.now(timezone.utc) - log.start_time).total_seconds():.2f}s")
    log.log("")
    ...
    log.log(f"Final: ω={final['omega']:.6f} F={final['F']:.6f} S={final['S']:.6f} C={final['C']:.6f} τ_R={final['tau_R']} κ={final['kappa']:.6f} IC={final['IC']:.6f}")
    log.log(f"Regime: {receipt['receipt']['regime_summary']['final_regime']} | OOR events: {receipt['receipt']['typed_boundaries']['oor_count']}")
    log.log("")
