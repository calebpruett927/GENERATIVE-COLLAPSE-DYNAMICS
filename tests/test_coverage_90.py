...
def test_minimal_cli_as_script():
    ...
    result = subprocess.run(
        [sys.executable, "-c", "from umcp.minimal_cli import main; exit(main())"],
        capture_output=True,
        text=True
    )
    assert result.returncode == 0
...
def test_structured_logger_operation_error():
    ...
    with pytest.raises(ValueError):
        with logger.operation("failing_op") as metrics:
...
def test_validator_observables():
    """Test validator validates observables."""
    from umcp.validator import RootFileValidator
...
def test_validator_checksums():
    """Test validator validates checksums."""
    from umcp.validator import RootFileValidator
    import hashlib
...
def test_validator_regime_classification():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (outputs_dir / "regimes.csv").write_text(
            f"regime_label,omega,F\nStable,{omega},{F}\n"
        )
...
def test_validator_checksum_file_missing():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        integrity_dir = tmppath / "integrity"
        integrity_dir.mkdir()
        (integrity_dir / "sha256.txt").write_text(
...
def test_validator_full_validation_with_files():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (tmppath / "outputs" / "regimes.csv").write_text(
            f"regime_label,omega,F\nStable,{omega},{F}\n"
        )
