...
def compute_valid_return_set(
    t: int, gamma_history: dict[int, tuple[float, float]], eta_phase: float = ETA_PHASE, W: int = W, delta: int = DELTA
) -> set[int]:
    """
...
def compute_kinematic_credit(tau_kin: int | KinSpecialValue, return_rate: float, T_crit: float = T_CRIT) -> float:
    """
...
def compute_sample_statistics(values: list[float]) -> tuple[float, float, bool]:
    """
...
def compute_K_stability(
    x_values: list[float], v_values: list[float], return_rate: float, sigma_max: float = SIGMA_MAX, v_max: float = V_MAX
...
class TestContractFreezeInvariants:
    ...
    def test_contract_snapshot_exact_match(self):
        ...
        assert expected["W"] == W
        assert expected["delta"] == DELTA
        assert expected["T_crit"] == T_CRIT
        assert expected["epsilon_log"] == EPSILON_LOG
        assert expected["eta_phase"] == ETA_PHASE
        assert expected["sigma_max"] == SIGMA_MAX
        assert expected["v_max"] == V_MAX
        assert expected["K_stable"] == K_STABLE
        assert expected["K_watch"] == K_WATCH
        assert expected["epsilon_p"] == EPSILON_P
        assert expected["epsilon_E"] == EPSILON_E
    ...
    def test_symbol_collision_guard_Omega_vs_omega(self):
        ...
        if kin_math_path.exists():
            ...
            if section_start != -1:
                ...
                # Should contain Ω
                assert "Ω" in section or "\\Omega" in section, "§10.2 should use Ω for angular velocity"
    ...
    def test_Lyapunov_delta_gamma_firewall(self):
        ...
        if kin_math_path.exists():
            ...
            if section_start != -1:
                ...
                assert has_delta_gamma or has_clarification, "§11 should clarify Δγ vs δ or use Δγ notation"
...
class TestConservationLaws:
    ...
    def test_conservation_tests_are_conditionally_applied(self):
        ...
        def momentum_conservation_test(p_series: list[float], F_ext: float) -> bool | None:
            """Returns None if not applicable, True/False otherwise."""
            if F_ext != 0:
                return None  # Not applicable

            # Check conservation
            return all(abs(p_series[i] - p_series[i - 1]) < EPSILON_P for i in range(1, len(p_series)))

        def energy_conservation_test(E_series: list[float], W_nc: float) -> bool | None:
            """Returns None if not applicable, True/False otherwise."""
            if W_nc != 0:
                return None  # Not applicable

            return all(abs(E_series[i] - E_series[i - 1]) < EPSILON_E for i in range(1, len(E_series)))
