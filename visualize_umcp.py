...
def load_ledger(repo_root: Path) -> pd.DataFrame | None:
...
def load_latest_receipt(repo_root: Path) -> dict[str, Any] | None:
...
def load_invariants(repo_root: Path) -> dict[str, Any] | None:
...
def load_regimes(repo_root: Path) -> dict[str, Any] | None:
...
def load_welds(repo_root: Path) -> pd.DataFrame | None:
...
def plot_omega_vs_curvature(df: pd.DataFrame, show_anomalies: bool = True):
    ...
    fig.add_annotation(
        x=0.019,
        y=df["curvature"].max() * 0.9,
        text="Stable",
        showarrow=False,
        font={"size": 12, "color": REGIME_COLORS["Stable"]},
    ...
    fig.add_annotation(
        ...
        text="Watch",
        showarrow=False,
        font={"size": 12, "color": REGIME_COLORS["Watch"]},
        bgcolor="rgba(255, 255, 255, 0.7)",
    )
    ...
    if df["omega"].max() >= 0.30:
        fig.add_annotation(
            ...
            text="Collapse",
            showarrow=False,
            font={"size": 12, "color": REGIME_COLORS["Collapse"]},
            bgcolor="rgba(255, 255, 255, 0.7)",
        )
    ...
    if show_moving_avg and len(df) >= 3:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                line={"color": "#388E3C", "width": 3, "dash": "dash"},
                opacity=0.7,
            ),
            row=3,
            col=1,
...
def display_latest_receipt(receipt: dict[str, Any] | None):
...
def display_current_invariants(invariants: dict[str, Any] | None):
    ...
    st.markdown(
        f"### Current Regime: <span style='color:{regime_color}; font-weight:bold;'>{regime}</span>",
...
def main():
    ...
    with st.spinner("Loading UMCP data..."):
        ...
        load_all_receipts(repo_root)
        invariants = load_invariants(repo_root)
        load_regimes(repo_root)
        load_welds(repo_root)
        ...
        with tab1:
            ...
            if show_advanced_stats and not ledger_df_filtered.empty:
                ...
                with col2:
                    st.metric(
                        "C Range",
                        f"{ledger_df_filtered['curvature'].min():.6f} - {ledger_df_filtered['curvature'].max():.6f}",
                    )
                with col3:
                    st.metric(
                        "S Range",
                        f"{ledger_df_filtered['stiffness'].min():.6f} - {ledger_df_filtered['stiffness'].max():.6f}",
                    )
